...
void drawN(int N,TH1F* hist[], const char *xtitle="", int rebin=-1,double xrange_low=-1,double xrange_high=-1,double id=-1, double ymax=-1,bool logy=false, bool logx=false, std::string graphname="untitled"){
  ...
  std::cout<<"hist0:"<<hist[0]->GetName()<<std::endl;
  std::cout<<"hist1:"<<hist[1]->GetName()<<std::endl;
  for(int i=0;i<N;i++){
  ...
  legend->AddEntry(hist[0], "H target");
  legend->AddEntry(hist[1], "non-H target");
